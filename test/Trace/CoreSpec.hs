...
spec = do
  ...
  describe "Sampler" $ do
    it "alwaysOnSampler returns RecordAndSample" $ do
      tid <- newTraceId
      runSampler alwaysOnSampler Nothing tid (mkTestSpanName "n") Internal mempty
        `shouldBe` RecordAndSample

    it "alwaysOffSampler returns Drop" $ do
      tid <- newTraceId
      runSampler alwaysOffSampler Nothing tid (mkTestSpanName "n") Internal mempty
        `shouldBe` Drop

    it "traceIdRatioSampler 1.0 always samples" $ do
      tids <- replicateM 100 newTraceId
      let s = traceIdRatioSampler 1.0
      all (\tid -> runSampler s Nothing tid (mkTestSpanName "n") Internal mempty
  ...
  describe "traceIdRatioSampler" $ do
    it "rate 0.0 samples nothing" $ do
      ...
      let s = traceIdRatioSampler 0.0
      all (\t -> runSampler s Nothing t (mkTestSpanName "n") Internal mempty == Drop) tids
        `shouldBe` True

    it "rate 1.0 samples everything" $ do
      tids <- replicateM 200 newTraceId
      let s = traceIdRatioSampler 1.0
      all (\t -> runSampler s Nothing t (mkTestSpanName "n") Internal mempty == RecordAndSample) tids
        `shouldBe` True
    ...
    it "is deterministic for the same trace-id" $ do
      ...
      runSampler s Nothing tid (mkTestSpanName "n") Internal mempty
        `shouldBe` runSampler s Nothing tid (mkTestSpanName "n") Internal mempty
  ...
  describe "parentBasedSampler" $ do
    ...
    it "sampled parent gives RecordAndSample regardless of root" $ do
      ...
      runSampler s (Just sampledCtx) tid (mkTestSpanName "n") Internal mempty
        `shouldBe` RecordAndSample
    ...
    it "unsampled parent gives Drop regardless of root" $ do
      ...
      runSampler s (Just unsampledCtx) tid (mkTestSpanName "n") Internal mempty
        `shouldBe` Drop
...
prop_parentBased_inherits_parent_decision = do
  ...
  let s      = parentBasedSampler alwaysOnSampler
      result = runSampler s (Just ctx) (scTraceId ctx) (mkTestSpanName "n") Internal mempty
...
prop_parentBased_no_parent_delegates = do
  ...
  let root   = traceIdRatioSampler rate
      ...
      direct = runSampler root   Nothing tid (mkTestSpanName "n") Internal mempty
      via    = runSampler pBased Nothing tid (mkTestSpanName "n") Internal mempty
  direct === via
